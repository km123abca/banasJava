...
public class SpaceShip extends Polygon
	...
	public int shipTrav=0;
	public int lShipTrav=50;
	public int lShipId=9999;
	...
	public SpaceShip(ArrayList<SpaceShip> spaceShips)
			...
			while(collision)
				{   ourbounds=this.getBounds();
					...
					this.setYCenter(yshift+20);
					double xshift= (double) (Math.random() * (gBWidth-40));
					this.setXCenter(xshift+20);
					collision=false;
	...
	public int shootnow()
		{  
			this.shipTrav+=1;
			if(this.shipTrav>20) this.shipTrav=0;
			if(this.shipTrav%5!=0) return 0;
			System.out.println("Shot fired from Ship No:"+String.valueOf(this.getShipId()));
			GameBoard.makeThatShootingSound();
	...
	public void exchangeSpeeds(SpaceShip otherShip)
		{	/*
			if( (this.lShipId==otherShip.getShipId())&&(Math.abs(shipTrav-lShipTrav)<10) )
					this.lShipId=otherShip.getShipId();
				else{
					this.lShipId=otherShip.getShipId();
					lShipTrav=shipTrav;*/
					GameBoard.makeThatExplodingSound();
			...
			otherShip.setYVelocity(yvelocity2);
			otherShip.setXVelocity(xVelocity2);
			//System.out.println("exchange Happended");
				//}
		}
	...
	public void move(ArrayList<SpaceShip> spaceShips,ArrayList<PhotonTorpedo> torpedoes)
		...
		//this.shipTrav+=1;
		//if(this.shipTrav>1000) this.shipTrav=0;
		for (SpaceShip otherShip:spaceShips)
		{
			...
			if(ourShip.intersects(hostileShip))
			{
				...
				for(int i=0;i<8;i++){
				otherShip.increaseXPos(otherShip.getXVelocity());
				otherShip.increaseYPos(otherShip.getYVelocity());
									}						
			}
		...
		if(this.getShipId()!=1){
			...
			//this.setMovingAngle(this.getRotationAngle()); //km here
		...
		if(distx!=8888.88)
			{   System.out.println("ship NO:"+String.valueOf(this.getShipId())+" senses danger moving now");
				...
				this.increaseXVelocity(0.1*this.shipXMoveAngle(this.getMovingAngle()) );
				this.increaseYVelocity(0.1*this.shipYMoveAngle(this.getMovingAngle()) );

			}
		else
			{  
					//this.setXVelocity(0);km here
					//this.setYVelocity(0);
									 
			}
						}
		this.increaseXPos(this.getXVelocity());		
		if(this.getXCenter()-25 < 0)
			{
			//this.setXCenter(gBWidth);
			this.bounceBack('x');this.stepAway();
			} 
		else
			if (this.getXCenter()+25 > gBWidth)
			{
		...
		this.increaseYPos(this.getYVelocity());
		if(this.getYCenter()-50 < 0)
			{
			this.bounceBack('y');
			this.stepAway();
			//this.setYCenter(gBHeight);
			} else
			if (this.getYCenter()+50 > gBHeight)
			{
			this.bounceBack('y');
			this.stepAway();
			//this.setYCenter(0);
