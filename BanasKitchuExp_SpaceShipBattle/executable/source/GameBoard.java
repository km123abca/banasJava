...
public class GameBoard extends JFrame{
	...
	public static void clearTorpedoes()
				{	boolean checker=true;
					for(PhotonTorpedo t1:torpedos)
						{if (t1.onScreen) 
							{checker=false;break;}
						}
					if(checker) torpedos.clear();
				...
				for (SpaceShip admiralKen:spaceShips)
					{
						...
						if(shipBounds.intersects(torpedoBounds)) 
							{	if (admiralKen.getShipId()==torpedo.getMotherShip()) continue;
								...
								if(admiralKen.getShipId()==1) GameBoard.makeThatExplodingSound();
							}
					}
